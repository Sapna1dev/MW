...
class ajaxCart extends HTMLElement {
    ...
    updateEvents(){
      ...
      this.querySelectorAll('[data-qty-input]').forEach(button => button.addEventListener('click', this.onQtyChange.bind(this)));
    }
    ...
    _updateCart(response, action){
      ...
      let cartHTML = response['template-cart'];
     ...
     */
    getCartData(action){
        const fetchCart = fetch(`${routes.cart_fetch_url}?sections=template-cart,header`);
